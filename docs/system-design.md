...
- **Performance:** затримка API-відповіді не повинна перевищувати 1 секунду (за винятком випадків із
  запитами до GitHub API).
- **Scalability:** система підтримує вертикальне та горизонтальне масштабування.
...
- **Observability:** логування та моніторинг подій.
- **Integrability:** система надає OpenAPI-документацію.
- **Disaster recovery:** швидке відновлення після збоїв або втрати даних.
...
- Взаємодія з GitHub API для отримання актуальних даних про релізи репозиторіїв.
- Взаємодія з PostgreSQL для постійного зберігання підписок.
- Взаємодія з Redis для кешування.
- Взаємодія з Notifier для відправки email.
- Взаємодія з Scheduler для планування подій.
